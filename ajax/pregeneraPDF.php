...
class PDF extends FPDF{
	...
	foreach($arr as $idcontrato){
		...
		$NominaTipoNomina = $_GET['idtiponomina'];
		...
		$pdf->Row(array('','', 'RECIBO DE NOMINA', $serie.'-'.$folio));
		$pdf->Ln(2);
		...
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(37,5,$_GET['diasPagados'],0,0,'L');		
		$pdf->SetFont('Arial','b',8);
		$pdf->Cell(25,5,"Fecha de Pago",0,0,'L');
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(37,5,$_GET['fechaPago'],0,0,'L');
		$pdf->Ln(4);
		$pdf->SetFont('Arial','b',8);
		...
		*/
		$pdf->SetXY(145,-45);
		$pdf->SetFont('Arial','b',8);
		$pdf->MultiCell(62,3,'RECIB� DE '.strtoupper($EmisorNombre).' EL TOTAL NETO SE�ALADO Y ESTOY DE ACUERDO CON LO INDICADO.',0,'C');
