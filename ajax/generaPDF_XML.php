...
class PDF extends FPDF{
	...
	function catalogo($tabla,$id){
		$conexion = new mysqli(HOST,USER,PASSWORD,BD);
		$query = "SELECT descripcion FROM ".$tabla." WHERE id='".$id."'";
	...
	$querya = "SELECT * FROM ptimbrado WHERE status=1 ORDER BY folio ASC";
	$sqla = $conexion->query($querya);
	while($arr = $sqla->fetch_assoc()){
		$serie = $arr['serie'];
		$folio = $arr['folio'];
		...
		$carga_xml = simplexml_load_file('CFDIS/RUVL810825597/'.$row['serie'].$row['folio'].'.xml');
		$ns = $carga_xml->getNamespaces(true);
		$carga_xml->registerXPathNamespace('c', $ns['cfdi']);
		$carga_xml->registerXPathNamespace('t', $ns['tfd']);
		$carga_xml->registerXPathNamespace('n', $ns['nomina12']);
				
		$cReceptor = $carga_xml->xpath('//c:Receptor');
		$ReceptorNombre = $cReceptor[0]['Nombre'];
		$ReceptorRFC = $cReceptor[0]['Rfc'];
		
		$nComprobante = $carga_xml->xpath('//c:Comprobante');
		$total = (float) $nComprobante[0]['Total'];
		$descuento = (float) $nComprobante[0]['Descuento'];
		$subtotal = (float) $nComprobante[0]['SubTotal'];
		
		$nReceptor = $carga_xml->xpath('//n:Receptor');
		$nip = $nReceptor[0]['NumEmpleado'];
		$NominaReceptorEstado = $nReceptor[0]['ClaveEntFed'];
		$NominaReceptorAntiguedad = $nReceptor[0][utf8_encode('Antigüedad')];
		$NominaReceptorInicioLab = $nReceptor[0]['FechaInicioRelLaboral'];
		$NominaReceptorCurp = $nReceptor[0]['Curp'];
		$NominaReceptorPeriodicidad = $pdf->catalogo('cperiodicidadpago', (string) $nReceptor[0]['PeriodicidadPago']);
		$NominaReceptorSalarioIntegrado = $nReceptor[0]['SalarioDiarioIntegrado'];
		$NominaReceptorDepartamento = $nReceptor[0]['Departamento'];
		$NominaReceptorRiesgo = $pdf->catalogo('criesgopuesto', (string) $nReceptor[0]['RiesgoPuesto']);
		$NominaReceptorSalariobase = $nReceptor[0]['SalarioBaseCotApor'];
		$NominaReceptorPuesto = $nReceptor[0]['Puesto'];
		$NominaReceptorTipoRegimen = $pdf->catalogo('ctiporegimen', (string) $nReceptor[0]['TipoRegimen']);
		if(isset($nReceptor[0]['Banco']))
			$NominaReceptorBanco = $nReceptor[0]['Banco'];
		if(isset($nReceptor[0]['NumSeguridadSocial']))
			$NominaReceptorNSS = $nReceptor[0]['NumSeguridadSocial'];
		$NominaReceptorTipoJornada =$pdf->catalogo('ctipojornada', (string) $nReceptor[0]['TipoJornada']);
		if(isset($nReceptor[0]['CuentaBancaria']))
			$NominaReceptorCuenta = $nReceptor[0]['CuentaBancaria'];
		if(isset($nReceptor[0]['Sindicalizado']))
			$NominaReceptorSindicalizado = $nReceptor[0]['Sindicalizado'];
		$NominaReceptorTipoContrato = $pdf->catalogo('ctipocontrato', (string) $nReceptor[0]['TipoContrato']);
		
		$nNomina = $carga_xml->xpath('//n:Nomina');
		$NominaTipoNomina = $nNomina[0]['TipoNomina'];
		$NominaDiasPagados = $nNomina[0]['NumDiasPagados'];
		$NominaFechaPago = $nNomina[0]['FechaPago'];
		$NominaFechaInicialPago = $nNomina[0]['FechaInicialPago'];
		$NominaFechaFinalPago = $nNomina[0]['FechaFinalPago'];
		...
		$nPercepciones = $carga_xml->xpath('//n:Percepciones');
		$TotalImporteGravado = $nPercepciones[0]['TotalGravado'];
		$TotalImporteExento = $nPercepciones[0]['TotalExcento'];
		
		if($carga_xml->xpath('//n:Percepcion')){
			$pdf->SetFont('Arial','b',8);
			...
			foreach($carga_xml->xpath('//n:Percepcion') as $row1){
				...
				$pdf->Row(array($row1['Clave'],$row1['TipoPercepcion'],$row1['Concepto'], number_format((float) $row1['ImporteGravado'],2,'.',','),number_format((float) $row1['ImporteExento'],2,'.',',')));
				if($carga_xml->xpath('//n:HorasExtra')){
					foreach($carga_xml->xpath('//n:HorasExtra') as $row2){
			...
			$pdf->Cell(20,5,number_format((float) $TotalImporteGravado,2,'.',','),'BT',0,'R');	
			$pdf->Cell(20,5,number_format((float) $TotalImporteExento,2,'.',','),'RBT',1,'R');
		}
		
		$pdf->Ln(3);
		
		$nDeducciones = $carga_xml->xpath('//n:Deducciones');
		$TotalOtrasDeducciones = (float) $nDeducciones[0]['TotalOtrasDeducciones'];
		$TotalImpuestosRetenidos = (float) $nDeducciones[0]['TotalImpuestosRetenidos'];
		$NominaTotalDeducciones = $TotalOtrasDeducciones + $TotalImpuestosRetenidos;
		
		if($carga_xml->xpath('//n:Deduccion')){
			$pdf->SetFont('Arial','b',8);
			...
			foreach($carga_xml->xpath('//n:Deduccion') as $row5){
				$pdf->SetWidths(array(20,20,134,20));
				$pdf->SetHeights(5);
				$pdf->SetFonts(array(array('Arial','',8),array('Arial','',8),array('Arial','',8),array('Arial','',8)));
				$pdf->SetBorders(array('L',0,0,'R'));
				$pdf->Row(array($row5['Clave'],$row5['TipoDeduccion'],$row5['Concepto'], number_format((float) $row5['Importe'],2,'.',',')));
			}
			$pdf->SetFont('Arial','b',8);
			$pdf->Cell(20,5,"",'LBT',0,'C');
			$pdf->Cell(20,5,"",'BT',0,'C');
		...
		$NominaTotalOtrosPagos = (float) $nNomina[0]['TotalOtrosPagos'];
		
		if($carga_xml->xpath('//n:OtroPago')){
			$pdf->SetFont('Arial','b',8);
			...
			foreach($arrOtrosPagos as $row6){
				...
				$pdf->SetFonts(array(array('Arial','',8),array('Arial','',8),array('Arial','',8),array('Arial','',8)));
				$pdf->SetBorders(array('L',0,0,'R'));
				$pdf->Row(array($row6['Clave'],$row6['TipoOtroPago'],$row6['Concepto'], number_format((float) $row6['Importe'],2,'.',',')));
			}
			$pdf->SetFont('Arial','b',8);
			$pdf->Cell(20,5,"",'LBT',0,'C');
			$pdf->Cell(20,5,"",'BT',0,'C');
			$pdf->Cell(134,5,"",'BT',0,'C');	
			$pdf->Cell(20,5,number_format(($NominaTotalOtrosPagos*1),2,'.',','),'RBT',1,'R');
		}	
		$pdf->Ln(3);
		$TotalIncidencias = 0;
		if($carga_xml->xpath('//n:Incapacidades')){
			$pdf->SetFont('Arial','b',8);
			$pdf->Cell(194,5,"INCAPACIDADES",1,1,'C');
			$pdf->Cell(20,5,"CLAVE",'L',0,'C');
			$pdf->Cell(154,5,"CONCEPTO",0,0,'C');
			$pdf->Cell(20,5,"IMPORTE",'R',1,'C');
			foreach($carga_xml->xpath('//n:Incapacidad') as $row7){
				$pdf->SetWidths(array(20,154,20));
				$pdf->SetHeights(5);
				$pdf->SetFonts(array(array('Arial','',8),array('Arial','',8),array('Arial','',8)));
				$pdf->SetBorders(array('L',0,0,'R'));
				$pdf->Row(array($row7['DiasIncapacidad'],$row7['TipoIncapacidad'], number_format((float) $row7['ImporteMonetario'],2,'.',',')));
			}
			$pdf->SetFont('Arial','b',8);
			$pdf->Cell(20,5,"",'LBT',0,'C');
